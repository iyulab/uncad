...
> **Where this lives now (2026-09-23).** This design was written and first
> implemented in the monolithic uncad of the `feat/0.3-readable` work (tag
> `archive/feat-0.3-readable`). The package it describes is now written by
> the `crates/uncad-export` crate on top of the three-repository split:
> the parser here (`uncad`), the model (`uncad-model`) and the renderer
> (`iron-render-cad`). The package format is the same; paths to modules in
> the text below (`crates/uncad/src/export.rs`, `crop.rs`, `svg.rs`, `png.rs`)
> refer to that earlier layout -- the export logic is in
> `crates/uncad-export/src/`, the crop rules in its `frame.rs`, and the
> rendering in `iron-render-cad`. Record ids now come from the model's
> entity ids. Section 10's status notes and measured numbers were taken on the
> earlier layout and have not been re-measured here.
