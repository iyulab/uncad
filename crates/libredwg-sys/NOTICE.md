...
- **What is here**: not all of LibreDWG. `vendor/libredwg/` holds only the
  ...
  LibreDWG's sources.
- **Modified**: **yes**, in five places. Both carry an `uncad local patch`
...
3. **`vendor/libredwg/src/in_dxf.c`**, 2026-09-23 -- the VERTEX_2D upgrade chain
   knows `AcDbPolygonMeshVertex`, the subclass marker AutoCAD writes on a
   polygon mesh's vertices. Without it the object stayed a VERTEX_2D whose
...
None of the changes alters LibreDWG's file formats, its API or its output for a
well-formed drawing; the last one corrects what it reads from one.
