...
//!
//! The fixture is written by the test itself, so this file does not depend on
...
const EXPECTED_UNSUPPORTED: &[&str] = &["DIMENSION", "TRACE"];

/// A minimal ASCII DXF: two drawable entities (LINE, CIRCLE) and two the
/// renderer leaves out (a TRACE, and a DIMENSION that carries no cached
/// block). TRACE deliberately comes first so that file order and sorted
/// order disagree.
