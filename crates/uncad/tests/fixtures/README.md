...
viewport-states and the radial fixtures on 2026-09-22) -- no third-party
drawing was copied, so they are redistributable under the repository's
GPL-3. All are R2000 (`$ACADVER AC1015`) text DXF with CRLF line endings,
between 470 and 4387 bytes, and above the 256-byte minimum LibreDWG's
`dwg_read_dxf` enforces.
