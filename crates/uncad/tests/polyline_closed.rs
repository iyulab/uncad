...
//! reference, so the numbers here (11 polylines, 10 closed) come from
//! AutoCAD's own writer, not from this project's output.
...
fn lwpolylines(db: &uncad::CadDatabase) -> Vec<(String, bool)> {
    let mut out: Vec<(String, bool)> = db
        ...
        .filter_map(|e| match e {
            Entity::LwPolyline(p) => match &p.common.source_handle {
                Ref::Resolved(handle) => Some((handle.clone(), p.closed)),
                other => panic!("an R2000 entity has a handle: {other:?}"),
            },
            _ => None,
        })
        .collect();
