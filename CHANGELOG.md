...
- `uncad redline <before> <after> -o <out.svg|out.png>` draws the difference between two
  ...
  `--omit`, `--frame changes` (the changes and some of the drawing around them, rather than the
  whole drawing) and, for a PNG, `--fit <px>`, writes a new file (never over an existing one), and answers with the
